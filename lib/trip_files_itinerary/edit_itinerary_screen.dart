...
class _EditItineraryScreenState extends State<EditItineraryScreen> {
  ...
  void initState() {
    ...
    _dayController = TextEditingController(text: _removeDayPrefix(widget.itinerary.day));
    
    // Parse time if it's a range
    if (widget.itinerary.time.contains('-')) {
      _isTimeRangeMode = true;
      List<String> timeParts = widget.itinerary.time.split(' - ');
      _startTimeController = TextEditingController(text: timeParts[0].trim());
      _endTimeController = TextEditingController(
        text: timeParts.length > 1 ? timeParts[1].trim() : ''
      );
    } else {
      _startTimeController = TextEditingController(text: widget.itinerary.time);
      _endTimeController = TextEditingController();
    }
    
    _activityController = TextEditingController(text: widget.itinerary.activity);
    _locationController = TextEditingController(text: widget.itinerary.location);
    _descriptionController = TextEditingController(text: widget.itinerary.description);
    _selectedIcon = widget.itinerary.icon;
    
    // Initialize file data and attachments from the itinerary
    if (widget.itinerary.hasAttachments) {
      for (var attachment in widget.itinerary.attachments) {
        _selectedFiles.add({
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      // Create an updated itinerary object
      itinerary_model.Itinerary updatedItinerary = itinerary_model.Itinerary(
        id: widget.itinerary.id, // Keep the original ID
      ...
      try {
        // Update in database
        await _dbHelper.updateItinerary(updatedItinerary);
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Error updating itinerary: ${e.toString()}',
                style: GoogleFonts.poppins(),
              ),
