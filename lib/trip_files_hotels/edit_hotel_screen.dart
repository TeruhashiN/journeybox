...
class _EditHotelScreenState extends State<EditHotelScreen> {
  ...
  // Available icons for selection (hotel-related)
  final List<MapEntry<String, IconData>> _availableIcons = [
    MapEntry('Hotel', Icons.hotel_outlined),
    MapEntry('Bed', Icons.bed_outlined),
    MapEntry('Apartment', Icons.apartment_outlined),
    MapEntry('Home', Icons.home_outlined),
    MapEntry('Villa', Icons.villa_outlined),
    MapEntry('Cabin', Icons.cabin_outlined),
    MapEntry('Resort', Icons.pool_outlined),
    MapEntry('Inn', Icons.business_outlined),
    MapEntry('Motel', Icons.local_hotel_outlined),
    MapEntry('Hostel', Icons.group_outlined),
  ];
  ...
  void initState() {
    ...
    _dayController = TextEditingController(text: _removeDayPrefix(widget.hotel.day));

    // Parse time if it's a range
    if (widget.hotel.time.contains('-')) {
      _isTimeRangeMode = true;
      List<String> timeParts = widget.hotel.time.split(' - ');
      _startTimeController = TextEditingController(text: timeParts[0].trim());
      _endTimeController = TextEditingController(
        text: timeParts.length > 1 ? timeParts[1].trim() : ''
      );
    } else {
      _startTimeController = TextEditingController(text: widget.hotel.time);
      _endTimeController = TextEditingController();
    }

    _activityController = TextEditingController(text: widget.hotel.activity);
    ...
    _selectedIcon = widget.hotel.icon;

    // Initialize file data and attachments from the hotel
    if (widget.hotel.hasAttachments) {
      for (var attachment in widget.hotel.attachments) {
        _selectedFiles.add({
        ...
        // Add file type to selected types
        _selectedFileTypes.add(attachment.fileType);
      }
      _isFileTypeSelectionMode = false;
    }
  }

  // Remove "Day " prefix if present for editing
  String _removeDayPrefix(String day) {
    if (day.toLowerCase().startsWith('day ')) {
      return day.substring(4); // Remove 'Day ' prefix
    }
    return day;
  }
  ...
  // Submit form and update in database
  Future<void> _submitForm() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      hotel_model.Hotel updatedHotel = hotel_model.Hotel(
        id: widget.hotel.id, // Keep the original ID
      ...
      try {
        // Update in database
        await _dbHelper.updateHotel(updatedHotel);
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Error updating hotel: ${e.toString()}',
                style: GoogleFonts.poppins(),
              ),
  ...
  Future<void> _selectStartTime() async {
    final TimeOfDay? pickedTime = await showTimePicker(
      context: context,
      initialTime: _parseTimeOfDay(_startTimeController.text) ?? TimeOfDay.now(),
  ...
  Future<void> _selectEndTime() async {
    final TimeOfDay? pickedTime = await showTimePicker(
      context: context,
      initialTime: _parseTimeOfDay(_endTimeController.text) ?? TimeOfDay.now(),
      builder: (BuildContext context, Widget? child) {
        return Theme(
          data: Theme.of(context).copyWith(
            colorScheme: const ColorScheme.light(
  ...
  TimeOfDay? _parseTimeOfDay(String timeStr) {
    ...
    try {
      // Handle format like "1:30 PM"
      final RegExp regExp = RegExp(r'(\d+):(\d+) ([AP]M)');
  ...
  // Process files from file picker result
